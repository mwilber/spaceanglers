...
class Score extends CI_Controller
{
	...
	function add($pFormat="html")
	{
	    ...
	    if($this->form_validation->run())
	    {
	        ...
	        // Validation passes
	        $nId = $this->score_model->Add($_POST);
	...
	function index($pFormat="html")
	{
		...
		$data['total_rows'] = $this->score_model->Get(array('count' => true));
		$data['records'] = $this->score_model->Get(array('sortBy'=>'scoreTimeStamp','sortDirection'=>'DESC'));
		$data['fields'] = $this->score_model->_fields();
		$data['pk'] = $this->score_model->_pk();
	...
	function TopTen(){
		$this->load->model('score_model');
		$data['records'] = $this->score_model->Get(array('sortBy'=>'scoreNumber','sortDirection'=>'DESC','limit'=>10,'offset'=>0));
		
		$this->load->view($this->uri->segment(1).'/'.$this->uri->segment(1).'_topten_index', $data);
	}
	
	function build($recordId){
		$this->load->model('score_model');
		$record = $this->score_model->Get(array($this->score_model->_pk() => $recordId));
	...
	function getconfig($recordId){
		$this->load->model('score_model');
		$record = $this->score_model->Get(array('facebook_page' => $recordId));
	    if(!$record) redirect($this->uri->segment(1));
	...
	function csv($pFormat="html")
	{
		...
		$data['total_rows'] = $this->score_model->Get(array('count' => true));
		$data['records'] = $this->score_model->Get(array('sortBy'=>'scoreTimeStamp','sortDirection'=>'ACS'));
		$data['fields'] = $this->score_model->_fields();
		$data['pk'] = $this->score_model->_pk();
	...
	function paginated($offset = 0)
	{
	    ...
	    $config['total_rows'] = $this->score_model->Get(array('count' => true));
		...
		$data[$this->uri->segment(1)] = $this->score_model->Get(array('sortBy'=>'order','sortDirection'=>'ASC','limit' => $perpage, 'offset' => $offset));
		$data['fields'] = $this->score_model->_fields();
		$data['pk'] = $this->score_model->_pk();
	...
	function details($pFormat="html", $pId){
		...
		$data['record'] = $this->score_model->Get(array('flashDataId'=>$pId));
	...
	{
		$this->load->model('score_model');
		$data['record'] = $this->score_model->Get(array($this->score_model->_pk() => $recordId));
		...
		{
	        // Validation passes
	        $_POST[$this->score_model->_pk()] = $recordId;
	        
	        if($this->score_model->Update($_POST))
	        {
	...
	function delete($recordId)
	{
	    ...
	    $this->score_model->Delete($recordId);
			...
			if( isset($jfbprof->id) ){
				if( !isset($jfbprof->first_name) ){
			    	...
			    	if(isset($_FILES['userfile'])){
						$imageData = $this->score_model->manageFile($_POST['facebook_page']);
					}
					if( !isset($imageData['error']) ){
				        ...
				        $tmpRec = $this->score_model->Get(array('facebook_page'=>$_POST['facebook_page']));
						if( count($tmpRec) > 0 ){
							$nId = $tmpRec[0]->flashDataId;
							$_POST[$this->score_model->_pk()] = $nId;
							$this->score_model->Update($_POST);
						}else{
							$nId = $this->score_model->Add($_POST);	
						}
